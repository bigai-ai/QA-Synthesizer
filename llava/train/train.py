...
import random
from process.syn_utils import load_syn_data, _load_json_or_jsonl
...
class DataArguments:
    ...
    syn_mode: Optional[str] = field(default='') # choose from ['', 'precise+informative']
    replace_with_blank_image_percent: int = field(default=0) # percentage of the images that will be replaced with blank image when training the synthesizer
    caption_path: Optional[str] = field(default='') # path to ALLaVA-Caption-VFLAN-4V.json
    precise_qa_path: Optional[str] = field(default='') # path to vflan_metadata.json
    informative_qa_path: Optional[str] = field(default='') # path to ALLaVA-Instruct-VFLAN-4V.json
    blank_image_path: Optional[str] = field(default='') # path to blank image
...
def preprocess_llama3(
    ...
    has_image: bool = False,
    syn_mode: str='',
    debug_id: int=0, # NOTE adamllm: for debug, we will print out the loss and tokens every 500 samples.
) -> Dict:
    conv = conversation_lib.default_conversation.copy()
    roles = {"human": conv.roles[0], "gpt": conv.roles[1]}
    ...
    for i, source in enumerate(sources):
        ...
        for j, sentence in enumerate(source):
            ...
            conv.append_message(role, sentence["value"])
        if len(syn_mode) > 0:
            # NOTE (adamllm): to force the synthesizer to end after the eos token (instead of the eot token)
            conversations.append(conv.get_prompt() + tokenizer.eos_token)
        else:
            conversations.append(conv.get_prompt())
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for i, rou in enumerate(re_rounds):
            ...
            if i > 0:
                round_len -= 1
                instruction_len -= 1
            # NOTE (adamllm): The following commented lines are intended for debugging the masking of the synthesizer. 
            # Uncomment and modify as needed during development or troubleshooting.
            if len(syn_mode) > 0:
                if i == 0:
                    # if debug_id % 500 == 0:
                    #     rank0_print(f'## [DEBUG INFO] debug_id: {debug_id}, syn_mode: {syn_mode}')
                    #     rank0_print(f'## the 1st round:\n{rou}<|eot_id|>') # add one more eot becase the round_len has been appended with one more token above.
                    #     rank0_print(f'## input ids where the loss will be ignored:\n{target[cur_len: cur_len + round_len]}') # we cannot use decode becase the first round has <image> which cannot be decoded.
                    # the first round on caption, we ignore the loss on all of it
                    target[cur_len: cur_len + round_len] = IGNORE_INDEX
                else:
                    # if debug_id % 500 == 0:
                    #     rank0_print(f'## the later rounds:\n{rou}<|eot_id|>')
                    #     rank0_print(f'## input tokens where the loss will NOT be ignored:\n{tokenizer.decode(target[cur_len: cur_len + round_len], skip_special_tokens=False)}')
                    pass # from the 2nd round, we compute loss on all tokens
            else:
                # if debug_id % 500 == 0:
                #     rank0_print(f'## debug_id: {debug_id}, normal MLLM fine-tuning mode, only compute loss on reponse')
                #     rank0_print(f'## current conversational round:\n{rou}')
                #     rank0_print(f'## input ids where the loss will be ignored:\n{parts[0]}') # we cannot use decode becase the first round has <image> which cannot be decoded.
                target[cur_len: cur_len + instruction_len] = IGNORE_INDEX 

            cur_len += round_len
        if len(syn_mode) > 0:
            cur_len += 1 # to ensure the loss of eos being computed
            # if debug_id % 500 == 0:
            #     rank0_print(f'## eos debug, target:\n{target}')
            #     rank0_print(f'## first tokens before eos: target[cur_len-5:cur_len]:\n{tokenizer.decode(target[cur_len-5:cur_len], skip_special_tokens=False)}')
            #     rank0_print(f'## tokens that will be ingored after the eos [cur_len:]:\n{tokenizer.decode(target[cur_len:], skip_special_tokens=False)}')
        target[cur_len:] = IGNORE_INDEX
...
def preprocess(
    ...
    if conversation_lib.default_conversation.version == "llama3":
        return preprocess_llama3(sources, tokenizer, has_image=has_image, syn_mode=syn_mode, debug_id=debug_id)
    # add end signal and concatenate together
    conversations = []
    for source in sources:
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        if len(data_path) > 0:
            self.list_data_dict = _load_json_or_jsonl(data_path, [], rank=local_rank)

        if len(data_args.syn_mode) > 0:
            rank0_print(f'data_args.syn_mode: {data_args.syn_mode}')
            self.list_data_dict = load_syn_data(data_args.caption_path, 
                                                data_args.precise_qa_path, 
                                                data_args.informative_qa_path, 
                                                data_args.blank_image_path,
                                                'train', 
                                                data_args.replace_with_blank_image_percent)
        if 'messages' in self.list_data_dict[0]:
            # means the data follows the shareGPT format, we need to transfer it to Llava's
            for entry in self.list_data_dict:
                messages = entry.pop("messages")
                assert messages[0]['content'].startswith('<image>')
                messages[0]['content'] = messages[0]['content'].replace('<image>', '<image>\n')
                entry['conversations'] = []
                assert len(messages)%2 == 0
                for idx in range(0, len(messages), 2):
                    assert messages[idx]["role"] == 'user' and messages[idx+1]["role"] == "assistant"
                    Q = messages[idx]['content']
                    A = messages[idx+1]['content']
                    entry['conversations'] += [{"from": "human", "value": Q},{"from": "gpt","value": A}]
                images = entry.pop('images')
                assert len(images) == 1
                entry['image'] = images[0]

        rank0_print(f'loaded data num: {len(self.list_data_dict)}')
        rank0_print(f'self.list_data_dict[0]: {self.list_data_dict[0]}')
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            # NOTE adamllm: there are a lot of invalid images in the med data, 
            # follow `LLaVA-Med_preview/llava/train/train.py` to catch errors
            try:
                if image_file == self.data_args.blank_image_path:
                    image_path = self.data_args.blank_image_path
                else:
                    image_path = os.path.join(image_folder, image_file)
                image = Image.open(image_path).convert('RGB')
            except Exception as exn:
                # randomly re-sample another index and get the item
                replace_with = random.Random(i).randrange(0, len(self.list_data_dict))
                print(f'{exn}, replace with: {replace_with}')
                return self.__getitem__(replace_with)
        ...
        data_dict = preprocess(
            ...
            syn_mode=self.data_args.syn_mode,
            debug_id=i, # for debug loss computation
            )
        if isinstance(i, int):
...
# NOTE adamllm: save callback for saving lora intermediate states
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        if 'mpt' in model_args.model_name_or_path:
            ...
            model = LlavaLlamaForCausalLM.from_pretrained(
                ...
                **bnb_model_from_pretrained_args,
                **model_max_length_args,
                mm_vision_tower=model_args.vision_tower # NOTE adamllm: override the mm_vision tower path in the model config
            )
    else:
    ...
    trainer = LLaVATrainer(model=model,
                           tokenizer=tokenizer,
                           args=training_args,
                           callbacks=[SaveCallback(training_args)], # NOTE adamllm: save callback for saving config files for lora
                           **data_module)
