"""
This is a mirror of https://github.com/microsoft/LMOps/blob/main/instruction_pretrain/utils/read_compre_pt.py
We removed some redundant ."""
import dataclasses
import copy
import random
...
FEWSHOT_PATTERNS = {
    ...
    "qa_w_option": [
        ...
        # "program_synthesis_dmcc_python_input_inversion"
        FewShotPattern(
            inputs="If this is the answer: {option_answer}\n What was the question?",
            targets="{question}\n{options_}",
            targets_prefix="Question: ",
            x_y_delimiter="\n",
            example_separator="\n\n"),
...
# --------------------------- CLASSIC RC FORMAT ---------------------------------
# modified based on AdaptLLM templates: LMOps/blob/main/adaptllm/utils/read.py
...
# modified based on: google-research/FLAN/blob/e9e4ec6e2701182c7a91af176f705310da541277/flan/v2/preprocessors.py#L588
...
def format_one_pt_rc(read_entry, pattern_dict):
    if len(read_entry['QA_list']) == 0:
        if read_entry['context'].strip() == '': print(f'!!!!!! empty context in read_entry: {read_entry} ')
    ...
    if one_pt_rc.strip() == '': print(f'!!!!!! empty one_pt_rc: {one_pt_rc}')
    return one_pt_rc